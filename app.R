...
ui <- fluidPage(
    ...
    p("Welcome! This website is a companion site to the outreach project", tags$strong("We can see our oceans from space - a tailored illustrated guide to promote engagement with Earth Observation"), "funded by the", tags$a(href="https://www.agu.org", "AGU"), "as part of celebrating 100 years of Earth and Space science."),
    ...
    h2("Step 1: Get access"),
    
   
    p("Obtain a free trial account from", tags$a(href="https://services.sentinel-hub.com/oauth/subscription", "their website."), "Once you have an account, log in, and go to the Configuration utility to copy the ID given in the WMS template (Service endpoint). To be able to download the satellite image you need to enter this ID. The script does not save any data, but to be able to make the pdf it will make a temporary copy of the satellite image you download."),
    
    textInput("caption", "Your Sentinel Hub INSTANCE ID here", "", width = "60%"),
    
    h2("Step 2: Choose your area"),
    ...
    selectInput("band", "Choose the satellite band you want to download",
                choices = c("Natural", "Vegetation", "MoistureIndex","Agriculture"), width = "60%"),
    p("(1) Natural will download a natural color image, (2) this image will indicate where vegetation is based on a colorized infrared image, (3) MoistureIndex gives you an indication about the water content of the ground, (4) Agriculture shows agricultural activities"),
    
    h2("Step 3: Choose your language"),
    selectInput("language", "Choose your poster language",
                choices = c("English", "French", "German"), width = "60%"),
    
    
    h2("Step 4: Add a title"),
    ...
    p("Find some examples and read more about the project on the", tags$a(href="https://noc.ac.uk/education/educational-resources/earth-observation-poster","NOC page.")),
    
    p("This shiny app was written by ", tags$a(href="https://heuschele.com/","heuschele.com."), "The source code is available on", tags$a(href="https://github.com/DerJH/EOmapmaker","Github."))
)
...
server <- function(input, output, session) {
    ...
    trara <- reactive({
        switch(input$band,
               "Natural" = "TRUE_COLOR",
               "Vegetation" = "FALSE-COLOR-VEGETATION",
    ...
    blabla <- reactive({
        switch(input$language,
               "English" = "NOC_english_map.png",
               "French" = "NOC_french_map.png",
               "German" = "NOC_german_map.png")
    })
    
    blabla2 <- reactive({
        switch(input$language,
               "English" = "NOC_english_frame.png",
               "French" = "NOC_french_frame.png",
               "German" = "NOC_german_frame.png")
    })
    ...
    observeEvent(input$go, {
        withProgress(message = 'Making plot', value = 0, {
            ...
            tryCatch({
                download.file(hereyougo,'satimage.png', mode = 'wb')
            },
            ...
            filename = paste(blabla())
            eobs <- image_read(filename)
            
            
            incProgress(7/n, detail = paste("Reading the images"))
            
            filename = paste(blabla2()) 
            overlay <- image_read(filename)
          
            satimage <- image_read("satimage.png")
            
            incProgress(8/n, detail = paste("Combining the images"))
            
            img <- c(satimage,overlay)
            img = image_flatten(img, operator = "Over")
